...
final class AdminRoleService
{
    ...
    {
        return $this->roles->all(\App\Support\Pagination::page($requestedPage));
    }
    public function permissions(): array { return $this->roles->permissions(); }
    public function options(): array { return $this->roles->activeOptions(); }
    ...
    public function save(array $input): array
    {
        ...
        if (($id !== null && ($id === false || $id < 1))
            ...
            || ($description !== null && (!is_string($description) || strlen($description) > 255))
            || !is_bool($active)
        ...
        try {
            return ['id' => $this->roles->save($id === null ? null : (int) $id,
                $code, trim($name), $description === null ? null : trim($description),
                $active, array_values(array_unique($permissions)))];
